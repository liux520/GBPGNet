...
if __name__ == '__main__':
    ...
    for ii in tqdm(range(0, 2000000)):
        idx = get_position_from_periods(ii, cumulative_period)
        current_weight = restart_weights[idx]
        nearest_restart = 0 if idx == 0 else cumulative_period[idx - 1]
        current_period = periods[idx]

        lr = eta_min + current_weight * 0.5 * (base_lr - eta_min) * (1 + math.cos(math.pi * ((-1 - nearest_restart) / current_period)))
    ...
    # weights = [1, 1, 1, 0.6, 0.6, 0.2, 0.2, 0.1]
    ...
    #             break
    #     # 使用余弦衰减策略更新学习率
    #     lr = cosine_decay(init_lr * lr_weight, i % 25, 25)
    #     lrs.append(lr)
    #
    # # 绘制迭代次数与学习率的曲线
    ...
    # plt.title('Cosine Decay with Periodic Weights')
    # plt.grid(True)
    # plt.legend()
    # plt.show()
