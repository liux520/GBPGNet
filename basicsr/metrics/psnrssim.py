...
import torchvision
import torch
from basicsr.metrics.psnr_ssim import reorder_image, to_y_channel
...
def _calculate_psnr_ssim_niqe(output, target, cal_psnr=True, cal_ssim=True, cal_niqe=False, rgb_range=1, crop_border=0,
                              input_order='CHW', test_y_channel=True, mean=(0, 0, 0), std=(1, 1, 1)):
    # denormalize:
    ...
    for i in range(b):
        if cal_psnr: psnr += calculate_psnr(output_np[i, :, :, :], target_np[i, :, :, :], crop_border=crop_border,
                                            input_order=input_order, test_y_channel=test_y_channel)
        if cal_ssim: ssim += calculate_ssim(output_np[i, :, :, :], target_np[i, :, :, :], crop_border=crop_border,
                                            input_order=input_order, test_y_channel=test_y_channel)
        if cal_niqe: niqe += calculate_niqe(target_np[i, :, :, :], crop_border=crop_border, input_order=input_order,
                                            convert_to='y')
